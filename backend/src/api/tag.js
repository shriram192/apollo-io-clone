...
router.get('/location', async (req,res) => {
  ...
  if(!(req.query['q_tag_fuzzy_name'] && req.query['kind'])) {
  ...
  try {
    const result = await axios.get(process.env.TAG_URL, { params: { ...req.query, api_key: process.env.API_KEY} })
...
router.post('/organization', async (req,res) => {
    ...
    if(!req.body['q_organization_fuzzy_name']) {
        return res.status(400).json({
            message: "Invalid API Input, does not contain the required fields!"
        })
    }
